...
?>
<html>
    <title>diaryson Log in Page</title>
    <head>
    ...
    *{ color: rgba(0, 0, 0, 1); margin: 0; padding: 0; -webkit-appearance: none; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; font-size: 1.7vmax;}
    ...
    <body>
        <div class="link" onclick="location.href='https://diaryson.info/'">Home page</div>
        <div class="loglink" onclick="location.href='../reg/'">Sign in page</div>
        ...
        if (isset($_POST['log'])){
            ...
            if ($result = mysqli_query($conn, "SELECT * FROM `users` WHERE `secret`='$secret'")){
                while($row = mysqli_fetch_array($result)){
                    if (empty($row['id'])){
                        echo 'NO SIMILAR ACCOUNT';
                    }
                    else {
                        $_SESSION['id']=$row['id'];
                        $_SESSION['type']='user';

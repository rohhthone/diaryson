...
$id=$_SESSION['talkid'];
                ...
                $ip = $_SERVER['REMOTE_ADDR'];$ipv=ip2long($_SERVER['REMOTE_ADDR']);
                    if ($sql = mysqli_query($conn, "INSERT INTO `mymsg` (userid,time,text,ip) VALUES ('$id', NOW(),'$message', '$ipv')")){
                        if (mysqli_query($conn, $sql)) { 
                            
                        } 
                        else {
                            ?>
                            <script>location.href='https://diarium.info/admin/message/talk/';</script>
                            <?php
                      }
                }
?>
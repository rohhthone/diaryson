...
$text=$_POST['changetext'];
$update="UPDATE info SET description='$text', WHERE name='$name'";
...
$ssql = "SELECT value FROM info WHERE name='$name'";
$rs = $conn->query($ssql);
if ($rs->num_rows>0){while($rrs=$rs->fetch_assoc()){$selected=$rrs['description'];$filedir='../file/'.$selected;unlink($filelink);}}
$update="UPDATE info SET description='$description',value='$logo', WHERE name='$name'";
$uresult = $conn->query($update);
}
$description=$_POST['changedescrption'];
$update="UPDATE info SET description='$description', WHERE name='$name'";
$uresult = $conn->query($update);
echo "Record changed successfully";
echo "<script>setTimeout(function(){window.history.back();},3000);</script>";
}

...
<html>
    ...
    *{ color: rgba(0, 0, 0, 1); margin: 0; padding: 0; -webkit-appearance: none; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif; font-size: 1.7vmax;}
    ...
    <body>
        <div class="link" onclick="location.href='../'">Back to list</div>
        <div class="name">Hello ADMIN! - Talk with <?php if ($resu = mysqli_query($conn, "SELECT * FROM `users` WHERE `userid`='$id'")){ while($r = mysqli_fetch_array($resu)){ echo $r['name']; }} ?></div>
        <div class="function">
        ...
        <div class="messagebox">
                    ...
                    <div class="body">
                        ...
                        if ($result = mysqli_query($conn, "SELECT * FROM `msg` WHERE `userid`='$id'")){
                            while($row = mysqli_fetch_array($result)){   
                                echo "<div class='msg'>".$row['text']."</div>";
                            }
                        } else {echo "<div class='msg'>You have no message</div>";}
                        ?>
                    </div>
                </div>
                    ...
                    <div class="body">
                        ...
                        if ($resul = mysqli_query($conn, "SELECT * FROM `mymsg` WHERE `userid`='$id'")){
                            while($ro = mysqli_fetch_array($resul)){
                                echo "<div class='msg'>".$ro['text']."</div>";
                            }
                        } else {echo "<div class='msg'>I do not send to you any messages</div>";}
                        ?>
                    </div>
                </div>
            </div>
            <div class="sendbox">
                <form action="sending/" method="post">
                    <textarea name="message" id="message" cols="30" rows="10" maxlength="6200"></textarea>
